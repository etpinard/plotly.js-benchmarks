...
var pathToSuites = path.join(__dirname, '..', 'suites');
...
if(suiteName) {
  var pathToResult = path.join(pathToSuites, suiteName, 'results.json');
  plotOne(pathToResult);
}
else {
  glob(pathToSuites + '*/results.json', function(err, files) {
